	...
	private AndroidDriver driver;
	 public StoreLogPage(AndroidDriver driver) {
		...
		public void takeAppLog() throws FileNotFoundException, IOException {
			File f= new File(".//SmAmp MaxLOG.txt");
			if (f.exists()) {
				f.delete();
				}
			else {
			...
			String projectRoot = System.getProperty("user.dir");
		     Runtime.getRuntime().exec("adb pull /storage/emulated/0/Download/SmAmp MaxLOG.txt"+projectRoot+"/SmAmp MaxLOG.txt");
		     
		     System.out.println("File saved at: " + projectRoot + "/SmAmp MaxLOG.txt");
